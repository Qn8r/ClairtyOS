...
class QuestifyWidgetProvider : AppWidgetProvider() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == ACTION_QUICK_COMPLETE) {
            val changed = runBlocking { completeNextRoutineFromExternalAction(context) }
            if (changed) updateAll(context)
        }
    }

    companion object {
        private const val ACTION_QUICK_COMPLETE = "com.example.questify.widget.COMPLETE_ONE"
        ...
        private fun updateWidgets(context: Context, manager: AppWidgetManager, ids: IntArray) {
            ...
            ids.forEach { id ->
                ...
                rv.setTextViewText(R.id.widgetTitle, "Journey Today")
